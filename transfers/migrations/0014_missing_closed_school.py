...
from transfers.models import School, Transfer

### START CONFIG ###
data_dir = BASE_DIR + '/data/processed/'
enrollment_filename = data_dir + 'missing_closed_school.csv' 
### END CONFIG ###
...
def load_schools(apps,schema_editor):
    ...
    for row in enrollment_csv:
        try:
            ...
            ts_from_home = Transfer.objects.filter(from_home_rcdts = rcdts)
            for t in ts_from_home:
                t.from_home_school = school

            ts_from_serving = Transfer.objects.filter(from_serving_rcdts = rcdts)
            for t in ts_from_serving:
                t.from_serving_school = school
